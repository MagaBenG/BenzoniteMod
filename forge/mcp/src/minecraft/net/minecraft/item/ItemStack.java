...
public final class ItemStack
{
    ...
    public boolean isItemDamaged()
    {
        ...
        if (getItem() != null)
        {
            damaged = getItem().isDamaged(this);
        }
    ...
    public List getTooltip(EntityPlayer par1EntityPlayer, boolean par2)
    {
        ...
        if (!multimap.isEmpty())
        {
            ...
            while (iterator.hasNext())
            {
                ...
                if (d0 > 0.0D)
                {
                    arraylist.add(EnumChatFormatting.BLUE + StatCollector.translateToLocalFormatted("attribute.modifier.plus." + attributemodifier.func_111169_c(), new Object[] {field_111284_a.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())}));
                }
                else if (d0 < 0.0D)
                {
                    d1 *= -1.0D;
                    arraylist.add(EnumChatFormatting.RED + StatCollector.translateToLocalFormatted("attribute.modifier.take." + attributemodifier.func_111169_c(), new Object[] {field_111284_a.format(d1), StatCollector.translateToLocal("attribute.name." + (String)entry.getKey())}));
                }
            }
        }
    ...
    public Multimap func_111283_C()
    {
        ...
        {
            object = this.getItem().func_111205_h();
        }
